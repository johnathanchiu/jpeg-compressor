...
def decompress_image(file_name, image_save):
    ...
    def decompress(input, dimx=0, dimy=0, qual=64, table=QUANTIZATIONTABLE, count=1, debug=False):
    ...
    newY = decompress(compressedY, dimx=s_length, dimy=s_width,
                      qual=quality_metric, table=table, count=1, debug=DEBUG)
    newCb = decompress(compressedCb, dimx=s_length, dimy=s_width,
                       qual=int(quality_metric*SAMPLE_RATIO), table=tablecr, count=2, debug=DEBUG)
    newCr = decompress(compressedCr, dimx=s_length, dimy=s_width,
                       qual=int(quality_metric*SAMPLE_RATIO), table=tablecr, count=3, debug=DEBUG)
